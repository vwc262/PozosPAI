...
namespace BoyQRGenerator
{
    ...
    public class QRCodeGenerator
    {
        ...
        public QRCode CreateQrCode(string plainText, ECCLevel eccLevel, bool utf8BOM = false)
        {
            ...
            var eccInfo = capacityECCTable.Where(x => x.Version == version && x.ErrorCorrectionLevel.Equals(eccLevel))
                .Single();
        ...
        private Polynom MultiplyGeneratorPolynomByLeadterm(Polynom genPolynom, PolynomItem leadTerm,
            int lowerExponentBy)
        {
            Polynom resultPolynom = new Polynom();
                ...
                for (int x = 0; x < 7; x++)
                {
                    if (alignmentPatternBaseValues[i + x] != 0)
                    {
                        for (int y = 0; y < 7; y++)
                        {
                            ...
                            {
                                Point p = new Point(alignmentPatternBaseValues[i + x] - 2,
                ...
                for (int x = 0; x < size; x = x + pixelsPerModule)
                {
                    for (int y = 0; y < size; y = y + pixelsPerModule)
                    {
                        ...
                        if (module)
                        {
                            ...
                            bmp.SetPixels(x, bmp.height - y - pixelsPerModule, pixelsPerModule, pixelsPerModule,
                                pixels);
                        }
                        else
                        {
